...
begin
  ...
  if Edit2.Text <> '' then
  begin
    fDM.TMKB.Filtered := False; // отключаем фильтр
    ed := Edit2.Text; // передаем в fDM новый текст
    fDM.TMKB.Filtered := True; // включаем фильтр
  end
...
begin
  ...
  begin
    if MessageDlg('Поменять заболевание?', mtConfirmation, [mbYes, mbNo], 0)
      = mrYes then
    begin
      fDM.TRab.Edit;
...
begin
  Shablon := ExtractFileDir(ParamStr(0)) + '\Шаблоны\pril3.doc';
...
var
  query: string;
begin
  if MessageDlg('Очистить МКБ10 у сотрудника?', mtConfirmation, [mbYes, mbNo],
    0) = mrYes then
  begin
    fDM.TRab.Edit;
