...
begin
  ...
  ExelTab.Workbooks.Open[st_in + '\Шаблоны\listskl.xls'];

  i:=6;

  fDM.QNeedSKL.SQL.Clear;
  fDM.QNeedSKL.SQL.Add
    ('SELECT fam, imya, fath, data_r, profes FROM rabotnik WHERE (data_proh BETWEEN :date AND :date2) AND (id_org=:org) AND (skl='+quotedstr('да')+')  AND (rabotnik.cat_osv not Like "п%") ORDER BY fam');
  ...
  ExelTab.ActiveWorkbook.SaveAs(st_in + 'Журнал\Разное\СКЛ\' +
