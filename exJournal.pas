...
begin
 ...
 begin
  ...
  ExelTab.Cells.Item[i,15]:=fDM.Query.FieldByName('dop_pri_usl').AsString; //доп в инд порядке
...
begin
 ...
 while not(fDM.Query.Eof) do
 begin
  Shablon:=ExtractFileDir(ParamStr(0))+'\Шаблоны\prot2.doc';
  ...
  s:=ExtractFileDir(ParamStr(0))+'\Журнал\ПротоколВК\'+inttostr(i)+'.doc';
 ...
 ShowMessage('Выгружено '+inttostr(i-1)+' файлов! Файлы находятся в каталоге '+ExtractFileDir(ParamStr(0))+'\Журнал\ПротоколВК\');
end;
