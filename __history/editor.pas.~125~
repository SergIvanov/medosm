...
begin
  ...
  Shablon := ExtractFileDir(ParamStr(0)) + '\Шаблоны\282н.doc';
...
var
  ...
  i: integer;
begin
  Shablon := ExtractFileDir(ParamStr(0)) + '\Шаблоны\pril7.doc';
  WordApplication1.Connect;
  WordApplication1.Visible := true;
  WordApplication1.Documents.Add(Shablon, EmptyParam, EmptyParam, EmptyParam);
  WordDocument1.ConnectTo(WordApplication1.ActiveDocument);
...
procedure TfEditor.Monthstr(S: string; Var so: string); // Дата прописью
const
...
begin
  try
    ...
    so := IntToStr(Day);
    so := so + ' ' + Mes[Month];
    so := so + ' ' + IntToStr(Year) + ' г.';
  except
    raise Exception.Create('"' + S + '"' + ' - такой даты нет!');
  end;
end;
...
var
  ...
  S: string;
begin
  Shablon := ExtractFileDir(ParamStr(0)) + '\Шаблоны\pril5.doc';
  WordApplication1.Connect;
  WordApplication1.Documents.Add(Shablon, EmptyParam, EmptyParam, EmptyParam);
  WordDocument1.ConnectTo(WordApplication1.ActiveDocument);
  WordApplication1.Visible := true;
...
var
  ...
  S: string;
begin
  Shablon := ExtractFileDir(ParamStr(0)) + '\Шаблоны\osm.doc';
  WordApplication1.Connect;
  WordApplication1.Visible := false;
  WordApplication1.Documents.Add(Shablon, EmptyParam, EmptyParam, EmptyParam);
  WordDocument1.ConnectTo(WordApplication1.ActiveDocument);
  ...
  if fDM.TRab.FieldByName('dop_pri_usl').AsString <> '' then
    WordInsertText('Допущен в индивидуальном порядке при условии: ' +
...
var
  ...
  S: string;
begin
  Shablon := ExtractFileDir(ParamStr(0)) + '\Шаблоны\prot1.doc';
  WordApplication1.Connect;
  WordApplication1.Visible := false;
  WordApplication1.Documents.Add(Shablon, EmptyParam, EmptyParam, EmptyParam);
  WordDocument1.ConnectTo(WordApplication1.ActiveDocument);
