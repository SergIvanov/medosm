...
begin
  ...
  if qry1.RecordCount=0 then Exit;
   ...
   ExelTab.Workbooks.Open[st_in + '\Шаблоны\aktdif2021.xls'];
  ...
  ExelTab.ActiveWorkbook.SaveAs(st_in + 'Журнал\экономист\акты\' + fDM.TOrg.FieldByName('imya_org').AsString + FormatDateTime('_yyyy_mm_dd', Date) + FormatDateTime('_hh_mm', time) + '.xls', EmptyParam, EmptyParam, EmptyParam, EmptyParam, EmptyParam, EmptyParam, false, EmptyParam, EmptyParam, EmptyParam);
...
begin
   ...
   st_in := ExtractFilePath(Application.ExeName);
   ExelTab := CreateOleObject('Excel.Application');
   ExelTab.Workbooks.Open[st_in + '\Шаблоны\akt6cdif2021.xls'];
