...
begin
  ...
  begin
    ...
    if user.us <> 'Администратор' then
      // Если не администратор, то выводить записи только конкретного пользователя
      fDM.TRab.CommandText :=
        'SELECT * FROM rabotnik WHERE (data_proh BETWEEN :date1 AND :date2) and (user='
        + quotedstr(us) + ')'
      // Иначе выводить записи всех пользователей
    else
      fDM.TRab.CommandText :=
...
begin
  ...
  SaveDBGridEhToExportFile(TDBGridEhExportAsXLS, DBGrid1,
    exe_dir + '\Журнал\Журнал_' + FormatDateTime('yyyy_mm_dd', Date) +
...
begin
  ...
  SaveDBGridEhToExportFile(TDBGridEhExportAsXLS, DBGrid1,
    exe_dir + '\Журнал\Журнал_' + FormatDateTime('yyyy_mm_dd', Date) +
    FormatDateTime('_hh_mm', time) + '.xls', true);
  Showmessage('База выгружена в файл ' + exe_dir + '\Журнал\Журнал_' +
...
begin
  if MessageDlg('Выйти из программы? ', mtConfirmation, [mbYes, mbNo], 0)
    = mrYes then
  begin
    Close;
  end;
