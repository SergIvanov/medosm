...
begin
  ...
  ExelTab.Workbooks.Open[st_in + '\Шаблоны\profosm.xls'];
  i := 5;
  k := 0;
  fDM.QStacRep.First;
  ExelTab.Cells.Item[2, 6] := 'с ' + DateToStr(DateTimePicker1.Date) +
  ...
  begin
    ...
    try
      ...
      ExelTab.Cells.Item[i, 9] := st;
      // showmessage('Неверный формат стажа работника!');
    end;
  ...
  ExelTab.ActiveWorkbook.SaveAs(st_in + 'Журнал\Стационар_' +
...
begin
  st_in := ExtractFilePath(Application.ExeName);
  ExelTab := CreateOleObject('Excel.Application');
  ExelTab.Workbooks.Open[st_in + '\Шаблоны\profcentr.xls'];
  i := 12;
  k := 0;
  fDM.QStacRep.First;
  ExelTab.Cells.Item[8, 2] := 'с ' + DateToStr(DateTimePicker1.Date) +
    '   по   ' + DateToStr(DateTimePicker2.Date);
  while not(fDM.QStacRep.Eof) do
  begin
    inc(k);
  ...
  ExelTab.ActiveWorkbook.SaveAs(st_in + 'Журнал\Профцентр_' +
    FormatDateTime('_yyyy_mm_dd', Date) + FormatDateTime('_hh_mm', time) +
    '.xls', EmptyParam, EmptyParam, EmptyParam, EmptyParam, EmptyParam,
    EmptyParam, false, EmptyParam, EmptyParam, EmptyParam);
  ExelTab.Quit;
  ShellExecute(Self.Handle, 'open', Pchar(st_in + '\Журнал'), nil, nil,
    SW_SHOWNORMAL);
end;
