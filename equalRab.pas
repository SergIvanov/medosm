...
begin
  ...
  ExelTab.Workbooks.Open[st_in + '\Шаблоны\equalSotr.xls'];
...
' FROM TempList LEFT JOIN rabotnik ON (TempList.dr = rabotnik.data_r) AND (TempList.ot = rabotnik.fath) AND (TempList.im = rabotnik.imya) AND (TempList.fam = rabotnik.fam) WHERE (rabotnik.fam is NULL)');
  ...
  ExelTab.ActiveWorkbook.SaveAs(st_in + 'Журнал\экономист\Списки сотрудников\Список соответствия' +
    ...
    ShellExecute(Self.Handle,'open', Pchar(st_in + '\Журнал\экономист\Списки сотрудников'),nil,nil,SW_SHOWNORMAL);
    ExelTab.ActiveWorkbook.Close;
    ExelTab.Quit;
    ExelTab:=0;
end;
